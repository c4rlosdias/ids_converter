...
if 'bsdd_loaded' not in st.session_state:
    st.session_state.bsdd_loaded = None
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("📥 Choose a XLSX file", type=['xlsx'], on_change= load)
    st.write(uploaded_file)
    # if load excel button is pressed   
    ...
    st.write(submit)
    # if load bSDD button is pressed
    if submit:
...
if st.session_state.mode is not None and st.session_state.load:
    ...
    with st.container():
        ...
        if st.session_state.mode == 'bsdd':
            search =  properties_search(domain, response.json())
            st.write(search)
            st.session_state.df = search
            #st.session_state.df = search if not uploaded_file  else st.session_state.df
                    ...
                    if st.session_state.mode == 'bsdd':
                        ...
                        if os.path.isfile('temp.xlsx'):
                            with open('temp.xlsx', 'rb') as file:
                                st.download_button('Download XLSX file', file, file_name=st.session_state.file_name.split('.')[0].strip() + '.xlsx', mime='xlsx' )
